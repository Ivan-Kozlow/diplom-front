...
export const FormCreateDescription: FC = () => {
	...
	const { mutate, isError, data, isSuccess } = useMutation({
		...
		mutationFn: (description: TypeFormCreateDescriptionFields['description']) =>
			formService.createDescription(description),
		retry: false,
	})
	...
	const onSubmit = handleSubmit((fields) => {
		if (fields.description.trim() === '') toast.error('Введите описание', { duration: 1400 })
		else if (fields.id.trim() === '') toast.error('Введите id', { duration: 1400 })
		else mutate(fields.description)
	})
